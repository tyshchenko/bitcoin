...
size_t CStats::MAX_STATS_MEM = 10*1024*1024; //10 MB
...
void CStats::addMempoolSample(int64_t txcount, int64_t dynUsage, int64_t currentMinRelayFee)
{
    uint64_t now = GetTime();
    {
        ...
        {
            //check memory usage
            int32_t memDelta = memusage::DynamicUsage(mempoolStats.vSamples) - MAX_STATS_MEM;
            if (memDelta > 0 && mempoolStats.vSamples.size())
            {
                ...
                size_t elementSize1 = memUsage/mempoolStats.vSamples.capacity();
                size_t elementSize = sizeof(mempoolStats.vSamples[0]);
                size_t itemsToRemove = ceil((memUsage-MAX_STATS_MEM)/sizeof(mempoolStats.vSamples[0]));
            ...
            int32_t memDelta1 = memusage::DynamicUsage(mempoolStats.vSamples) - MAX_STATS_MEM;
            mempoolStats.vSamples.shrink_to_fit();
            int32_t memDelta2 = memusage::DynamicUsage(mempoolStats.vSamples) - MAX_STATS_MEM;
...
mempoolSamples_t CStats::mempoolGetValuesInRange(uint64_t &fromTime, uint64_t &toTime, int intervalSec)
{
    LOCK(cs_stats);

    // if emtpy, return directly
