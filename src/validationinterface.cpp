...
struct MainSignalsInstance {
    ...
    boost::signals2::signal<void (const CBlockIndex *, bool fInitialDownload)> UpdatedBlockHeaderTip;
    boost::signals2::signal<void (const CTransactionRef &)> TransactionAddedToMempool;
    boost::signals2::signal<void (const uint256 &, CTransactionRef &)> FindTransaction;
...
void RegisterValidationInterface(CValidationInterface* pwalletIn) {
    ...
    g_signals.m_internals->FindTransaction.connect(boost::bind(&CValidationInterface::FindTransaction, pwalletIn, _1, _2));
...
void UnregisterValidationInterface(CValidationInterface* pwalletIn) {
    ...
    g_signals.m_internals->FindTransaction.disconnect(boost::bind(&CValidationInterface::FindTransaction, pwalletIn, _1, _2));
...
void CMainSignals::UpdatedBlockHeaderTip(const CBlockIndex *pindexNew, bool fInitialDownload) {
    m_internals->UpdatedBlockHeaderTip(pindexNew, fInitialDownload);
}
...
void CMainSignals::FindTransaction(const uint256 &hash, CTransactionRef &txRef) {
    m_internals->FindTransaction(hash, txRef);
}
