...
CStats::CStats() : statsEnabled(false), maxStatsMemory(0)
{
    ...
    mempoolStatsCollector = new CStatsMempool(STATS_COLLECT_INTERVAL);
}

CStats::~CStats()
{
    delete mempoolStatsCollector;
}
...
void CStats::collectCallback()
{
    ...
    if (mempoolStatsCollector->addMempoolSamples(maxStatsMemory)) {
...
void CStats::setMaxMemoryUsageTarget(size_t maxMem)
{
    ...
    mempoolStatsCollector->setMaxMemoryUsageTarget(maxMem);
}

CStatsMempool * CStats::mempoolStats()
{
    return mempoolStatsCollector;
}
