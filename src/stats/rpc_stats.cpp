...
UniValue getmempoolstats(const JSONRPCRequest& request)
{
    ...
    uint64_t timeFrom = 0;
    std::vector<unsigned int> groups = CStats::DefaultStats()->mempoolStats()->getPercisionGroupsAndIntervals();

    UniValue groupsUni(UniValue::VARR);
    for (unsigned int i = 0; i < groups.size(); i++) {

        MempoolSamplesVectorRef samples = CStats::DefaultStats()->mempoolStats()->getSamplesForPercision(i, timeFrom);
