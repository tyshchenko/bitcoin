...
typedef std::shared_ptr<MempoolSamplesVector> MempoolSamplesVectorRef;
...
private:
    ...
    std::vector<MempoolSamplesVectorRef> vSamplesPerPrecision; //!<multiple samples vector per percision level
    std::vector<size_t> vMaxSamplesPerPrecision; //!<vector with the maximum amount of samples per percision level
    std::vector<size_t> vTimeLastSample; //!<vector with the maximum amount of samples per percision level
...
public:
    ...
    /* get mempool percision groups and its time interval-target */
    std::vector<unsigned int> getPercisionGroupsAndIntervals();

    /* get mempool samples from a given percision group index(!) */
    MempoolSamplesVectorRef getSamplesForPercision(unsigned int percision, uint64_t& fromTime);
};
