    ...
    while (nBytes > 0) {
        ...
        if (vRecvMsg.empty() || vRecvMsg.back()->Complete()) {
            if (m_encryption_handler && m_encryption_handler->ShouldCryptMsg()) {
                vRecvMsg.emplace_back(MakeUnique<NetCryptedMessageEnvelope>(m_encryption_handler, Params().MessageStart(), SER_NETWORK, INIT_PROTO_VERSION));
        ...
        if (msg->Complete()) {
            ...
            if (msg->m_type == NetMessageType::PLAINTEXT_ENCRYPTION_HANDSHAKE && !msg->VerifyHeader()) {
            ...
            } else if (msg->m_type == NetMessageType::BIP151_ENVELOPE) {
                // keep old message (unique_ptr) in this scope until the decompose loop is done
                NetMessageBaseRef oldmsg = std::move(vRecvMsg.back());
                vRecvMsg.pop_back();
                ...
                while (oldmsg->vRecv.size()) {
                    ...
                    if (!cmsgref->DecomposeFromStream(oldmsg->vRecv)) {
                        LogPrint(BCLog::NET, "Invalid inner message peer=%i, disconnecting\n", GetId());
                        return false;
                    }
                    cmsgref->nTime = nTimeMicros;
                    RecordRecvBytesPerMsgCmd(cmsgref->GetCommandName(), cmsgref->GetMessageSizeWithHeader());
                    vRecvMsg.push_back(std::move(cmsgref));
                }
            } else {
                ...
                if (nBytes > 0)
                {
                    ...
                    if (notify) {
                        ...
                        for (; it != pnode->vRecvMsg.end(); ++it) {
                            ...
                            //never process envelope messages
                            assert(it->get()->m_type != NetMessageType::BIP151_ENVELOPE);
                        }
                        {
...
void CConnman::PushMessage(CNode* pnode, CSerializedNetMsg&& msg)
{
    ...
    if (should_crypt) {
        ...
        nTotalSize += pnode->m_encryption_handler->GetAADLen() + pnode->m_encryption_handler->GetTagLen() + serialized_command_size + sizeof(uint32_t) /* inner message size */;
    } else {
    ...
    if (should_crypt) {
        ...
        serialized_envelope.reserve(sizeof(uint32_t) + serialized_command_size + sizeof(uint32_t) + nMessageSize);
        uint32_t envelope_payload_length = serialized_command_size + 4 + nMessageSize;

        CVectorWriter{SER_NETWORK, INIT_PROTO_VERSION, serialized_envelope, 0, envelope_payload_length, msg.command, (uint32_t)nMessageSize};
        ...
        pnode->m_encryption_handler->EncryptAppendMAC(serialized_envelope);
        {
            LOCK(pnode->cs_vSend);
