...
void PeerLogicValidation::InitializeNode(CNode *pnode) {
    ...
    if (!pnode->fInbound) {
        // try to encrypt channels if node has the NODE_ENCRYPTED service flag or if it was added manually
        // TODO: don't try for encryption if it has previously failed
        if (((pnode->nServices & NODE_ENCRYPTED) || pnode->m_manual_connection) && gArgs.GetBoolArg("-netencryption", DEFAULT_ALLOW_NET_ENCRYPTION)) {
            // send a stealth encryption request
            connman->SendStealthEncryptionHandshakeData(pnode);
        } else {
...
{
    /* handling stealth encryption handshake */
    if (pfrom->nVersion == 0 && msg_type == NetMessageType::PLAINTEXT_ENCRYPTION_HANDSHAKE) {
        LogPrint(BCLog::NET, "Stealth encryption handshake payload received, peer=%i\n", pfrom->GetId());
        ...
        if (!pfrom->fInbound && pfrom->m_encryption_handler) {
            ...
            // stealth encryption handshake response
            connman->SendStealthEncryptionHandshakeData(pfrom);
            if (!pfrom->m_encryption_handler->ProcessHandshakeRequestData(handshake_msg)) {
                LogPrint(BCLog::NET, "Invalid handshake received, peer=%d\n", pfrom->GetId());
                return false;
            }
    ...
    LogPrint(BCLog::NET, "received%s: %s (%u bytes) peer=%d\n", msg_type == NetMessageType::BIP151_MSG ? " encrypted" : "", SanitizeString(strCommand), vRecv.size(), pfrom->GetId());
