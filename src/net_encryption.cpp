...
{
    // first 4 bytes are the LE uint32 message length
    // the most significant bit (bit 8 at third byte [<<31]) indicates to the
    // counterparty that the next message will be using the next key (rekey) with
    // resetted nonce

    if (data_in_out[0] & (1u << 31)) {
        // length is only allowed up to 2^31
        return false;
    }
