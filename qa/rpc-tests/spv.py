...
class SPVTest (BitcoinTestFramework):
    ...
    def wait_wallet_non_validation_sync(self, node, hash):
        timeout = 20
        while timeout > 0:
            if node.getwalletinfo()['nvs_bestblock_hash'] == hash:
                return
        assert(timeout>0)
    ...
    def run_test(self):
        ...
        # non validation sync must be possible due to the -nonvalidationsync mode
        self.wait_wallet_non_validation_sync(self.nodes[1], node0bbhash)
        
        # best block should be available at this point due to non-validation sync
